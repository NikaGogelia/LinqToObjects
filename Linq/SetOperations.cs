...
namespace Linq
{
	...
	public static class SetOperations
	{
		...
		public static IEnumerable<char> UnionOfQueryResults()
		{
			...
			var productInitials = products.Select(p => p.ProductName[0]);
			var customerInitials = customers.Select(c => c.CompanyName[0]);
			return productInitials.Union(customerInitials);
		}
		...
		public static IEnumerable<char> IntersectQueryResults()
		{
			...
			var productInitials = products.Select(p => p.ProductName[0]);
			var customerInitials = customers.Select(c => c.CompanyName[0]);
			return productInitials.Intersect(customerInitials);
		}
		...
		public static IEnumerable<char> DifferenceOfQueries()
		{
			...
			var productInitials = products.Select(p => p.ProductName[0]);
			var customerInitials = customers.Select(c => c.CompanyName[0]);
			return productInitials.Except(customerInitials);
		}
	}
}