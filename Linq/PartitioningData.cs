...
namespace Linq
{
	...
	public static class PartitioningData
	{
		...
		public static IEnumerable<(string customerId, int orderId, DateTime orderDate)> CustomersTake()
		{
			...
			.SelectMany(c => c.Orders, (customer, order) =>
				new { customer.CustomerId, order.OrderId, order.OrderDate })
			.OrderBy(x => x.OrderDate)
			.Take(3)
			.Select(x => (x.CustomerId, x.OrderId, x.OrderDate)) 
			.ToArray();
		}
		...
		public static IEnumerable<(string customerId, int orderId, DateTime orderDate)> CustomersSkip()
		{
			...
			return customers
				.Where(c => c.City == "Washington")
				.SelectMany(c => c.Orders, (customer, order) => new { customer.CustomerId, order.OrderId, order.OrderDate })
				.Skip(2)
				.Select(x => (x.CustomerId, x.OrderId, x.OrderDate));
		}
